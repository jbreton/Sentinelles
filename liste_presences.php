...
$query = 'SELECT id_presence, IFNULL(membre.nom, presence.nom) AS nom, IFNULL(membre.prenom, presence.prenom) AS prenom, courriel, membre.id_membre, groupe_vitesse.description, est_present, date_confirmation
FROM presence
...
while($row = $result->fetch_assoc()) {
    echo '<tr><td>'.htmlentities($row['prenom']).' '.htmlentities($row['nom']).($row['id_membre'] != '' ? ' '.$row['courriel'].' ('.$row['id_membre'].')' : '').'</td><td>'.$row['description'].'</td><td>'.$row['date_confirmation'].'</td>';
