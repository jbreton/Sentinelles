...
$query = 'SELECT courriel.id_courriel, courriel.objet, courriel.message, courriel.id_activite, courriel.inclure_lien, envoi_courriel.id_envoi, membre.nom, membre.prenom, membre.courriel FROM envoi_courriel INNER JOIN courriel ON courriel.id_courriel = envoi_courriel.id_courriel INNER JOIN membre ON membre.id_membre = envoi_courriel.id_membre WHERE envoi_courriel.envoye = "N" LIMIT 1;';
